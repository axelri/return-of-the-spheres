...
class font_data:
    def __init__ (self, facename, pixel_height):
        ...
        try:
            ...
            ft = freetype.Font(facename, ptsize=pixel_height)
        except:
...
def make_dlist (ft, ch, list_base, tex_base_list):
    ...
    if ch < 33 or ch > 126:
        glyph_width = 0
        glyph_height = 0
        glyph = pygame.Surface((0, 0))
    else:
        metrics = ft.get_metrics(chr(ch))[0]
        min_x, max_x, min_y, max_y, _not, _used = metrics 
        rect = ft.get_rect(chr(ch))
        print "'{}'".format(chr(ch))
        ...
        print "height"
        print glyph_height
        glyph = pygame.Surface((glyph_width, glyph_height))
        ft.render_to(glyph, (0,glyph_height), chr(ch), pygame.Color(0,0,0,0))
